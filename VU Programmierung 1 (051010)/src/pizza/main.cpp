...
ostream& operator<<(ostream &o, const Pizza &pizza) {
  return pizza.Print(o);
}

bool Topping::operator==(const Topping &rop) const { return rop.name == name; }

ostream& operator<<(ostream &o, const Topping &topping) {
  return topping.Print(o);
}
